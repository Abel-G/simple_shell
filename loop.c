...
* if the command is echo + $$ or echo $? or $environment
* Return: 0 if the comparisim matches or 1 if it doesnot match
*/
...
* Return: void
*/
int loop(char *envp[], char *copy_PATH, char *copy_PWD, char *get_stdin)
{
char *buffercopy, *argv_here[100];
int i, status = 0, m = 0, j = 0;
...
{
does_file_exist(copy_PATH, argv_here);
status = _process(argv_here, copy_PATH);
}
}
}
}
