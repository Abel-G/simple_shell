...
* @returnStatus: This is the return status of the process
* Return: return status
*/
int wait_fun(pid_t pid, int child_count, int returnStatus)
{
if (child_count)
{
child_count--;
...
* @path: PATH
* Return: return Status of most recent process
*/
...
pid_t pid;
char *copyforhash;
copyforhash = malloc(sizeof(char *) * 100);
...
if (flag == 1)
does_file_exist(path, &argv[first_element]);
pid = fork();
...
return (wait_fun(pid, child_count, returnStatus));
}
